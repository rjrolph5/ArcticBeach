...
Plot modelled water levels (ERAI-forced), assuming no sea ice cover.  The full 
timeseries produced in this script (modelled water levels and other data (sea 
surface temperature, winds, waves)) are subsequently masked in another script.

Veslobogen cliffs of Svalbard. 
...
# Input lat/lon of offshore site, which is chosen from the script: 
# generate_grid_maps_and_choose_forcing_grid_cells_veslobogen.py. (The RA 
# grid cell selected for winds can be visualized on a map there) These lat lon 
# should be the same as in the 
# make_mask_based_on_seaice_threshold_ERAI_input_community.py.  Note when masking 
# the output used in this script (e.g the sea ice concentration data used for making 
# water levels produced by winds should come from the same grid cell as the wind 
# data comes from).
