...
#from addcyclic_updated import addcyclic

# This is a plotting script to identify the offshore grid cell for each community

# At the moment, there are separate scripts for each study site but they can easily 
# be combined into one script with a user-input community name and lat lon.

###### set paths ######

# path where you save the lat lon and index information of the grid cells selected 
#in this script.
...
year = 2014 # choose any year where the reanalysis data is available, just need an example file that has the grid.

##### input lat lon of site where retreat rates are measured #####

lat_site = 77 # [degrees North]
lon_site = 15.5 # [positive means degrees East, negative is degrees West]
...
# have to correct the lons from the mask to the same convention as is used for the sicn variable. 
...
np.save(basepath + 'veslobogen/lat_site_ERAI_veslobogen.npy',lat_site_ERAI)
np.save(basepath + 'veslobogen/lon_site_ERAI_veslobogen.npy',lon_site_ERAI)
...
lat_offshore_site_ERAI_veslobogen = lats[lat_offshore_idx]
lon_offshore_site_ERAI_veslobogen = lons[lon_offshore_idx]

np.save(basepath + 'veslobogen/lat_offshore_site_ERAI_veslobogen.npy',lat_offshore_site_ERAI_veslobogen)
np.save(basepath + 'veslobogen/lon_offshore_site_ERAI_veslobogen.npy',lon_offshore_site_ERAI_veslobogen)
...
#plt.text(px_site_label, py_site_label, 'label', fontsize=16, fontweight='bold',color='r')
...
plt.savefig(plot_basepath + 'study_sites/grid_veslobogen' + '.png', bbox_inches='tight', dpi=300)
plt.show()
