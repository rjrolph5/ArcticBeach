...
######### this script finds the offshore grid cell the sea ice concentration data is taken from to take the mask.  ############
...
# draw grid lines
m_zoom.drawmeridians(np.arange(np.min(lons_mask),np.max(lons_mask)+0.75,0.75), color='r') # last value is exclusive so have to add 0.75
m_zoom.drawparallels(np.arange(-90, 90.75,0.75), color='r')
