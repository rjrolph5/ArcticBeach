...
fig.tight_layout()
fig.savefig(plot_path + 'modelled_vs_measured_wl_' + community_name + '.png', bbox_inches='tight',dpi=300)
plt.show()
...
plt.plot(tide_gauge_obs_wl, label = 'Prudhoe Bay tide gauge', color='b')
ax.tick_params('x',rotation=90,pad=1)
...
df_gauge_prudhoe_all_years = tide_gauge_obs_wl.copy()
...
for yr in year_range:
	...
	#inds_where_ice = np.where(np.isnan(modelled_water_levels_erai_sample_year))[0]
	tide_gauge_obs_wl_yr = tide_gauge_wl_sample_year[first_timestamp_open_water_model:last_timestamp_open_water_model]
	...
	df_gauge_prudhoe_all_years = df_gauge_prudhoe_all_years.append(tide_gauge_obs_wl_yr, ignore_index=False)
...
n_modelled_gauge, bins_modelled_gauge, patches_modelled_gauge = plt.hist(x=df_gauge_prudhoe_all_years_dropped_nan,density= True, bins='auto',color='b', alpha=0.7, rwidth=0.85, label = 'Observed')
n_modelled, bins_modelled, patches_modelled = plt.hist(x=df_modelled_water_levels_with_offset_erai_all_years_dropped_nan,density= True, bins='auto',color='r', alpha=0.7, rwidth=0.85, label = 'Modelled')

props = dict(boxstyle='round', facecolor='wheat',alpha=0.5)
ax.text(0.62,0.95,'Prudhoe Bay \n(2007 - 2016)', transform=ax.transAxes, fontsize=14, verticalalignment='top', bbox=props)

ax.legend(loc='upper left',prop={'size': 12})
ax.set_ylabel('Frequency', fontsize=25)
ax.set_xlabel('Water level [m]', fontsize=25)
ax.tick_params(axis='both', which='major', labelsize=15)
ax.set_ylim(0,3.2)
ax.set_xlim(-1.01, 1.7)
fig.tight_layout()
plt.savefig(plot_path + 'modelled_vs_measured_wl_' + community_name + '_histogram_with_offset.png', bbox_inches = 'tight', dpi=300)
plt.show()
