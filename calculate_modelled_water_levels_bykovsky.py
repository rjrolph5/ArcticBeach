...
Plot modelled water levels (ERAI-forced).

Mamontovy Khayata site:  site 3 ("C") on Bykovsky Peninsula in Overduin et al. (2007).

Use the full timeseries produced in this script that assumes no sea ice as an input for the script that masks these water level data 
...
plot_path = '/home/rrolph/erosion_model_output_figures_too_large_for_github/bykovsky/'

npy_path = basepath + 'input_data/storm_surge/Mamontovy_Khayata/'

bathymetry_ifile = npy_path + 'depth_bykovsky.npy'

# Input lat/lon of offshore site. this is produced in make_mask_with_extra_figures_single_year_sicn_threshold_ERAI_bykovsky_mamontovy_hayata.py (the RA grid cell selected for winds can be visualized on a 
# map there) These lat lon should be the same as in the make_mask_based_on_seaice_threshold_ERAI.py if masking the output used in this script (e.g the sea ice concentration data used for making water 
...
# option to save these input parameters you used to calculate the water level (from both the met forced and ERAI forced) so that they are read in by the input_file_[community_name].py at the same time as 
# their corresponding water levels.
#input_array_to_produce_water_levels = np.array([lat_site, lon_site, shorenormal_angle_from_north, storm_surge_threshold])
#np.save(npy_path + 'input_array_to_produce_modelled_water_levels_mamontovy_hayata.npy', input_array_to_produce_water_levels)
